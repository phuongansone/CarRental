...
import static common.RequestParam.CAR_ID;
...
public class UpdateCartServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int carId = StringUtil.parseInt(request.getParameter(CAR_ID), -1);
        int index = CarService.searchCarInCart(cart, carId);
        
        if (index > -1) {
            int oldQuantity = cart.get(index).getQuantity();
            int quantity = StringUtil.parseInt(request.getParameter(QUANTITY), oldQuantity);
            
            cart.get(index).setQuantity(quantity);
        }
        
        session.setAttribute(CommonAttribute.CART, cart);
        session.setAttribute(CommonAttribute.UPDATED, true);
